...
package com.example.tourismapp.qrcode;
...
public class QrcodeActivity extends Activity implements SurfaceHolder.Callback,
    ...
    private QrcodeDecodeThread mDecodeThread = null;
    private QrcodeCameraConfigurationManager mConfigManager;
    ...
    protected void onResume() {
        super.onResume();
        mConfigManager = new QrcodeCameraConfigurationManager(this);
        mFinderView = (QrcodeFinderView) findViewById(R.id.finderView);
        mSurfaceView = (SurfaceView) findViewById(R.id.preview_view);
        if (mHandler == null) {
            mHandler = new Handler(this);
            // コード認識用スレッドを開始
            mDecodeThread = new QrcodeDecodeThread(this, new QrcodeResultPointCallback(mFinderView));
            mDecodeThread.start();
        }
...
//            _Log.d(TAG, "decoded [" + text + "]");
    ...
    public PlanarYUVLuminanceSource buildLuminanceSource(byte[] data, int w, int h) {
        ...
        int xy[] = {0,0};
        mFinderView.getLocationOnScreen(xy);

        int t = (w < h) ? w: h;
        int left = (w - t) / 2;
        int top = (h - t) / 2;
        //int width = mFinderView.getLayoutParams().width;
        //int height = mFinderView.getLayoutParams().height;
        int width = t;
        int height = t;
        return new PlanarYUVLuminanceSource(data, w, h, left, top, width, height, false);
        //return new PlanarYUVLuminanceSource(data, w, h, top, left, height, width, false);
        //return new PlanarYUVLuminanceSource(data, w, h, 588, 60, 600, 600, false);
        //return new PlanarYUVLuminanceSource(data, w, h, 100, 100, 100, 100, false);
    }    
